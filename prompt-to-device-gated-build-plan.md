...
| 1 — Physical reference | Blocked | TBD | TBD | Needs a printer and the parts. Recommended first machine: a printer with a local API the bench agent already speaks — Prusa MINI+ (PrusaLink) or any Klipper/OctoPrint printer. Bambu A1 mini is cheaper but its adapter is not implemented. |
